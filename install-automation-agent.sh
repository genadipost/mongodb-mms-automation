...
if [[ ! -v MMS_BASE_URL_INTERNAL ]]; then
  VARIABLES_NOT_SET="\nMMS_BASE_URL_INTERNAL"
fi

if [[ ! -v MMS_GROUP_ID ]]; then
  VARIABLES_NOT_SET="$VARIABLES_NOT_SET\nMMS_GROUP_ID"
fi

if [[ ! -v MMS_AGENT_APIKEY ]]; then
  VARIABLES_NOT_SET="$VARIABLES_NOT_SET\nMMS_AGENT_APIKEY"
fi

if [[ ! -v AGENT_VERSION ]]; then
  VARIABLES_NOT_SET="$VARIABLES_NOT_SET\nAGENT_VERSION"
fi

if [[ -v VARIABLES_NOT_SET ]]; then
  echo -e "The following environment varibles are not set:\n----------"
  echo -e $VARIABLES_NOT_SET
  echo -e ----------
  echo -e "Exiting..."
  exit 1
fi
